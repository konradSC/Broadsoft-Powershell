#Gather User information from an Enterprise
...
$manifest = "C:\Temp\Logs\UserReport$enterpriseID$(get-date -format `"yyyyMMdd`").csv"
$Global:userInfo = @()
$Global:userList = @()
...
Get-UserGetListInServiceProviderRequest $global:sessionID $enterpriseID
#get the Users
...
# Put the Users into an array
    ...
    foreach ($userarray in $Global:userList) {
        ...
        foreach ($s in $_) {
                ...
                Add-Member -InputObject $f -MemberType NoteProperty -Name Location -Value $s.physicalLocation
        }
        ...
        $v = $t -join ", "
        #Add-Member -InputObject $f -MemberType NoteProperty -Name ServicePack -Value (New-object System.Collections.Arraylist)
